...
cineplexRouter.post(
  "/create-menu",
  [registerMenuMulter],
  expressAsyncHandler(createMenu)
);
cineplexRouter.get("/branches", expressAsyncHandler(getBranch));
cineplexRouter.get("/api-key", expressAsyncHandler(getAPIKey));
cineplexRouter.get("/screenings", expressAsyncHandler(getScreening));
cineplexRouter.get(
  "/screenings-lengkap",
  expressAsyncHandler(getScreeningLengkap)
);
