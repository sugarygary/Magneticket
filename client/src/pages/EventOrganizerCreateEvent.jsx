      ...
      <div>
        ...
        <div className="px-10">
          <div className="biruTua p-10 mb-10 rounded w-full text-white">
            <p>Nama Event</p>
            <input className="w-full px-2 py-2 abuInput outline-none mb-2 rounded" type="text" placeholder="Masukkan nama event" onChange={((e)=> {setNama(e.target.value)})}/>
            <p>Venue</p>
            <input className="w-full px-2 py-2 abuInput outline-none mb-2 rounded" type="text" placeholder="Masukkan venue" onChange={((e)=> {setVenue(e.target.value)})}/>
            <p>Address</p>
            <input className="w-full px-2 py-2 abuInput outline-none mb-2 rounded" type="text" placeholder="Masukkan Address" onChange={((e)=> {setAddress(e.target.value)})}/>
            <p>Tanggal Event</p>
            <input className="w-full px-2 py-2 abuInput outline-none mb-2 rounded" type="date" onChange={((e)=> {setTanggal(e.target.value)})}/>
            <p>Unggah Surat Verifikasi Event</p>
            <input className="px-2 py-2 mb-2" type="file" onChange={((e)=> {handleSurat(e.target.files[0])})}/>
            <p>Deskripsi Event</p>
            <textarea className="w-full md:w-full p-2 border resize-y abuInput outline-none mb-2 rounded" placeholder="Massukkan deskripsi event" onChange={((e)=> {setDeskripsi(e.target.value)})}></textarea>
            ...
            {kategori.length > 0 &&
              <div>
                {kategori.map((k, index)=> {
                  return (<div key={index} className="flex text-white w-full gap-2 mt-5">
                    <input type="text" className="w-4/12 px-2 py-2 abuInput outline-none rounded" value={k.namaKategori} disabled/>
                    <input type="text" className="w-4/12 px-2 py-2 abuInput outline-none rounded" value={k.hargaTiket} disabled/>
                    <input type="text" className="w-4/12 px-2 py-2 abuInput outline-none rounded" value={k.slotTiket} disabled/>
                  </div>)
                })
            ...
            {boolAddKategori &&
              <CardKategoriTiketEvent handleKategoriTicket={handleKategoriTicket} kategoriTiket={kategoriTiket} hargaTiket={hargaTiket} slotTiket={slotTiket} handleHargaTicket={handleHargaTicket} handleSlotTicket={handleSlotTicket}></CardKategoriTiketEvent>
            }
            <div className="cursor-pointer text-center w-full px-2 py-2 biruMuda outline-none mr-2 rounded mb-2" onClick={(()=>{tambahKategori()})}>Tambah Kategori</div>
            <p>Unggah Informasi Kategori/Zona</p>
            <input className="px-2 py-2 mb-2 mb-2" type="file" onChange={((e)=> {handleInformasiKategori(e.target.files[0])})}/>
            <div className="flex mb-2">
              <input className="mr-2" type="checkbox" checked={syaratKetentuan}
              onChange={handleCheckboxChange}/> Saya setuju dengan<p className="text-blue-400 ml-2"> Kebijakan privasi Magneticket</p>
            </div>
              { errorMessage != null &&
            ...
            <button className="biruMuda w-full py-2 rounded">Buat Jadwal Konser</button>
          </div>
        </div>
      </div>
    </form>
